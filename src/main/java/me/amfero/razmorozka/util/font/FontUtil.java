...
public class FontUtil
{
	...
	{
		return customFont() ? (Razmorozka.customFontRenderer.getStringWidth(text) + 3) : fontRenderer.getStringWidth(text);
	}
	...
	public static void drawString(String text, double x, double y, int color)
	{
		if (customFont())
		{
			Razmorozka.customFontRenderer.drawString(text, x, y - 1, color, false);
		}
		else
		{
	...
	public static void drawStringWithShadow(String text, double x, double y, int color)
	{
		if (customFont())
		{
			Razmorozka.customFontRenderer.drawStringWithShadow(text, x, y - 1, color);
		}
		else
		{
	...
	public static void drawCenteredStringWithShadow(String text, float x, float y, int color)
	{
		if (customFont())
		{
			Razmorozka.customFontRenderer.drawCenteredStringWithShadow(text, x, y - 1, color);
		}
		else
		{
	...
	public static void drawCenteredString(String text, float x, float y, int color)
	{
		if (customFont())
		{
			Razmorozka.customFontRenderer.drawCenteredString(text, x, y - 1, color);
		}
		else
		{
	...
	{
		return customFont() ? (Razmorozka.customFontRenderer.fontHeight / 2) - 1 : fontRenderer.FONT_HEIGHT;
	}
