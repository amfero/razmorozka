	...
	public static boolean canPlaceBlock(BlockPos pos)
	{
		...
		if (isBlockNotEmpty(pos))
		{
			...
			for (EnumFacing enumFacing : enumFacings)
			{
				...
				if (!emptyBlocks.contains(mc.world.getBlockState(pos.offset(enumFacing)).getBlock()) && mc.player.getPositionEyes(mc.getRenderPartialTicks()).distanceTo(new Vec3d(pos.getX() + 0.5D + (double) enumFacing.getFrontOffsetX() * 0.5D, pos.getY() + 0.5D + (double) enumFacing.getFrontOffsetY() * 0.5D, pos.getZ() + 0.5D + (double) enumFacing.getFrontOffsetZ() * 0.5D)) <= 4.25D)
				{
					return true;
				}
