...
public class Hud extends Module {
	...
	public Hud(String name, String description, Category category)
	{
    ...
    public void onRender(RenderGameOverlayEvent.Post event) {
		...
		if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
			if((ArrayList.getBooleanValue())) {
	            ...
	            for (final Module mod : Razmorozka.moduleManager.getModules()) {
	                if (mod.isEnabled()) {
	            ...
	            for (final String name : list) {
	            	CustomFontRenderer fr = Razmorozka.customFontRenderer;
			...
			if((Watermark.getBooleanValue())) {
				...
				Date date = new Date();
				String waterma = "RAZMOROZKA | " + formatter.format(date) + " | " + server + " | " + ping;
				Gui.drawRect(X, Y - 2, X + Razmorozka.customFontRenderer.getStringWidth(waterma) + 7, Y + H - 3, new Color(24,14,60, 185).getRGB());
				RenderUtil.drawGradientSideways(X, Y - 2, X + Razmorozka.customFontRenderer.getStringWidth(waterma) + 7, Y, new Color(64,41,213,255).getRGB(), new Color(124,9,77, 255).getRGB());
